...
export default function Home() {
    ...
    function getDomainName(urlString) {
        const urlWithoutProtocol = urlString.replace(/^(https?:\/\/)?(www\.)?/i, '');
        const domainName = urlWithoutProtocol.split('/')[0];
        return domainName;
    }
    ...
    return (
        <>
            ...
            <main>
                ...
                {/* Select File */}
                {links.length === 0 &&
                    <section className="section selectFile">
                        <input
                            ...
                            hidden
                        />
                        <label className="UploadBtn" htmlFor="fileID">
                            ...
                            <span>Select Your Bookmark File To Read.</span>
                        </label>
                    </section>}

                {/* Show Bookmark Links */}
                {links.length > 0 &&
                    <section className="section showLinks">
                        <div className="links">
                            {links.length > 0 && (
                                links.map((a) => (
                                    <a key={a.id} href={a.link} target="_blank">
                                        <div className="link">
                                            {   
                                                a.icon
                                                && (<Image src={a.icon} width={18} height={18} alt={a.title} loading="lazy" />)
                                                || (<div className="imgPlaceholder"></div>)
                                            }
                                            <p>
                                                {a.title ? a.title : getDomainName(a.link)}
                                            </p>
                                        </div>
                                    </a>
                                ))
...
/* 
 * created by hixvm
 * https://twitter.com/hixvmx
 * https://github.com/hixvmx
 */